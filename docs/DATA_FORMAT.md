...
C++ queries use text to avoid a JSON dependency; the oracle/checker reads JSON independently. Raw and canonical data were unchanged during the C++ handoff. The collection date is unknown. The reader does not access raw data to verify hashes on every query. Byte-identical output is not guaranteed across library/projection versions. The C++ algorithms are still stubs; the historical Python A*/hierarchical logic has not been fixed.
